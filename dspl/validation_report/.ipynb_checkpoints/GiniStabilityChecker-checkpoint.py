...
class GiniStabilityChecker(Checker):
    ...
    def _to_excel(self, df: pd.DataFrame, sheet_name: str, plot=False) -> None:
        ...
        for col in ["delta_train_vs_valid", "delta_train_vs_OOT",
                    "delta_train_vs_test","delta_train_vs_OOT_psi"]:
            if col in df.columns:
                excelWriter.set_col_cond_format(df, (0, 0), col,
                                                upper=-0.15, lower=-0.1,
                                                order="reverse")
